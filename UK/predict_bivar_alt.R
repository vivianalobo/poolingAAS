## predict = Extrapolacao
### y é a matriz de log taxas observada, ajustada no modelo
### m0 e C0 sao as distribuicoes inicias das medias e covariancias usadas no modelo
### h é o horizonte de previsao (idades inteiras)
### Ft e Gt sao a estrutura do modelo
### delta é o fator de desconto usado
### ages é o vetor das idades ajustadas
### V é parametro retornado pelo ffbs do gibbsV_corrigido
predict_bivar <- function(y, m0, C0, h, V, Ft, Gt, delta, ages){
  
  N = nrow(y)  ### y é a matriz de log taxas observada, ajustada no modelo
  q = ncol(y)
  p = length(m0) 
...
## Teste - modelo bivar ajustado com fator de desconto variando no tempo
#a <- predict_bivar(fit, h = 10)
